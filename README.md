...
## The demo: five industries, one engine

The landing page offers five demo territories of 20 real companies each - 100 accounts in all. Click an industry box to open its ranked board, then click any account for the source behind its signals:
...
4. **[Industrial Machine Health](https://eric-hastie.github.io/territory-radar/machine-health/)** - run as if selling an industrial machine health platform (condition monitoring, CMMS, energy management). The ICP is mid-market manufacturers with multiple plants, real rotating equipment, and chronic maintenance-staffing pressure.
5. **[Software Test Automation](https://eric-hastie.github.io/territory-radar/test-automation/)** - run as if selling an AI-powered test automation platform (autonomous testing for web, mobile, and API). The ICP is mid-market software companies shipping fast where QA is the bottleneck - the tell is a req list full of manual QA engineers and SDETs.
...
**Illustrative demos only.** All five boards run against unnamed hypothetical products; no real vendor is named, and none is affiliated with, nor has endorsed, this project. The companies, job postings, funding rounds, and leadership moves are real, verified July 13 to 19, 2026 against live ATS job boards (Greenhouse / Lever / Ashby / Workday and others) and public sources; the scoring and tiering are this tool's own. A live deployment runs against a real book of business in a **private** repo.
...
Each territory's accounts live in `data/territories/<slug>/latest.csv`, with dated snapshots (`YYYY-MM-DD.csv`) alongside for momentum. `build.py` recomputes every score and tier from the CSV signal columns, then generates the whole site in one pass: the landing page, the five territory boards, and the legacy pages. A weekly cloud agent re-verifies signals, rewrites the CSVs, snapshots them, runs `build.py`, and pushes - so the live demo stays current without me touching it during the week.
